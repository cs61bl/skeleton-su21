public class ArrayOperationsTest {
    ...
    public static boolean testNaturalRuns() {
        int[] A = {1, 3, 7, 5, 4, 6, 9, 10};
        int[][] firstValues = ArrayOperations.naturalRuns(A);
        int[][] afterNat1 = {{1, 3, 7}, {5}, {4, 6, 9, 10}};
        if (!check(afterNat1, firstValues)) {
            return false;
        }

        int[] B = {4, 7, 9, 0, 16, 2, 0, -1};
        int[][] secondValues = ArrayOperations.naturalRuns(B);
        int[][] afterNat2 = {{4, 7, 9}, {0, 16}, {2}, {0}, {-1}};
        if (!check(afterNat2, secondValues)) {
            return false;
        }
    ...
    public static void main(String[] args) {
        ...
        if (testNaturalRuns()) {
            System.out.println("testNaturalRuns passed!");
        } else {
            System.out.println("testNaturalRuns failed!");
        }
    }
}